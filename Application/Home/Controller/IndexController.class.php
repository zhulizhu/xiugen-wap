...
class IndexController extends HomeController
{
    ...
    public function index()
    {
        if (IS_CLI) {
            ...
            $game = I('game');
            $user = session('user');
            $user = M('members')->find($user['uid']);
            $this->assign('user', $user);
            $this->assign('game', $game);
            $this->assign('lott', $lott);
            if (IS_POST) {
    ...
    public function main()
    {
        ...
        $notice = M('important_notice')->where(array('enable' => 1))->order('id desc')->select();
...
//		$this->ajaxReturn($lastNo,'JSON');
    ...
    public final function getHistoryData()
    {
        ...
        $history = M('data')->where(array('type' => $this->type))->order('number desc,time desc')->limit(10)->field('time,number,data')->select();
        if(I('api')){
            $history = M('data')->where(array('type' => $this->type))->order('number desc,time desc')->field('time,number,data')->select();
            $total = count($history);//总条数
            $num = 15;//每页显示条数
            $page = I('page');
            $cpage = empty($page) ? 1 : $page;//当前页
            $pagenum = ceil($total / $num);//总页数
            $offset = ($cpage - 1) * $num;//开始去数据的位置
            $history = M('data')->where(array('type' => $this->type))->order('number desc,time desc')->field('time,number,data')->limit($offset,$num)->select();
        }
