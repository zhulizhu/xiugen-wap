...
class RecordController extends HomeController{
	...
	public final function betInfo(){
        ...
        if (IS_POST) {
			$bet['title'] = $this->types[$bet['type']]['title'];
			$bet['old_id'] = $this->types[$bet['type']]['old_id'];
			$bet['play_name'] = $this->playeds[$bet['playedId']]['name'];
			if($bet['isDelete']==1){
				$bet['state'] = '已撤单';
			...
			$bet['betCont']=$bet['mode'] * $bet['beiShu'] * $bet['actionNum'] ;
			$bet['fanDianCount'] = number_format(($bet['fanDian']/100)*$bet['betCont'], 4) ;
			$bet['yingKui'] = $bet['bonus'] - $bet['betCont'] - $bet['fanDianCount'];
			show_api_json(1,'投注详情',$bet);
            $this->json($this->view->get());
        } else {
	...
	public final function hemaiInfo(){
        ...
        if (IS_POST) {
            ...
            $this->display('Team/hemai_detail');
        }
	}
}
