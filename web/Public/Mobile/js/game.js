$(function(){
	...
	//合买
	$('#bet-confirm-mobile-s').unbind('click');
	$('#bet-confirm-mobile-s').bind('click',function(){
		if(document.getElementById('amount')=='undefined' || parseFloat($('#amount').text())<0.01){
			winjinAlert('请先输入投注号码',"alert");
			return false;
		}
		$('#hm_feng').val(parseInt($('#amount').html()));
		$('#hemaiMask').css('display', 'block');
	});
	$('#hemai-button').unbind('click');
	$('#hemai-button').bind('click', gamePostCode2);
});
...
function gamePostCode2(){
	...
	var tipString='确定要对第'+actionNo['actionNo']+'期发起合买？';
...
function gamePostedCode(err, data){
	if(err){
		if('您的可用资金不足，是否充值？'==err){
			winjinAlert('您的可用资金不足，正在跳转到充值页面。。',"alert");
			setTimeout(function(){
				window.location='/index.php?s=/mobile/user/recharge';
			},2000);
		}else{
			winjinAlert(err,"alert");
		}
		...
		$('#num-select .code.checked').removeClass('checked');
		$('#count').text('0');
		$('#amount').text('0.00');
