...
(function($) {
    var wait_close_timer, wait_show_timer;
    /** just tip **/
    ...
    justTools.prototype = {
        ...
        setEvent:function(){
            ...
            if(self.set.events =="mouseover" || self.set.events =="onmouseover" || self.set.events =="mouseenter"){
                var _elem = this.elem;
                if(Number(self.set['wait_close'] || -1) > 0){
                    var _tip_elem = $(".just-tooltip");
                    ...
                    _elem.hover(function(){
                        clearTimeout(wait_close_timer);
                    },function () {
                        _close();
                    });

                    _tip_elem.hover(function(){
                        _elem.addClass("on");
                        clearTimeout(wait_close_timer);
                    },function () {
                    ...
                    function _close() {
                        wait_close_timer = setTimeout(function () {
                            _tip_elem.remove();
                        }, self.set.wait_close);
                    }
                }else{
                    _elem.on("mouseout, mouseleave",function(){
                        self.close();
                    });
                }
            }
        ...
        setContent:function(){
            var max_style = ";z-index:" + this.set.z_index;
            if(Number(this.set.max_width) > 0){
                max_style = ";max-width:" + this.set.max_width + "px";
            }
            this.obj = $("<div id=" + this.set.id + " class='just-tooltip " + this.set.theme + "'" +
                "style='width:" + this.set.width + max_style + "'>" +
                "<div class='just-con'>" + this.set.contents + "</div>" + "<span class='just-" + this.set.gravity + "'></span></div>");
        ...
        init:function(){
            ...
            if(Number(this.set.wait_show) > 0){
                wait_show_timer = setTimeout(function () {
            ...
            function _create() {
                self.set.events = e.type;
                e.stopPropagation();
                self.setContent();
                self.setPosition();
                if(typeof self.set.init === 'function'){
                    self.set.init(self);
                }
        ...
        update: function(options){
            ...
            if(!options['confirm']){
                this.obj.find('.just-confirm').hide();
            }
            if(options['content']){
                this.obj.find('.just-con').html(options['content']);
            }
            this.setPosition();
        }
    };
