...
class UserController extends AdminController {
    ...
    public function index(){
		...
		if(I('regPath'))
			$map['regPath'] = base64_decode(I('regPath'));
		
        //$list   = $this->lists('Members', $map , 'uid asc');
		$map['admin'] = 0;
		$map['isDelete'] = 0;
		$data = M('Members')->where($map)->order('regTime desc')->select();
		$list = array();
		$i=0;
		foreach($data as $d)
		{
        ...
        if(IS_POST){
            ...
            if($data){
                ...
                if($lastid=$Config->add($data)){
					...
					$data['parents2']='admin>'.$data['username'];
					$Config->save($data);
	...
	public function edit(){
        if(IS_POST){
            ...
            if($data){
				...
				if(I('parentPath')!=I('oldparentPath')){
					...
					foreach($parent_array as $par){
						...
						$parent_path2 = $parent_path2.'>'.$par;
					}
					$parent_path = substr($parent_path,1);
					$parent_path2 = substr($parent_path2,1);
					$data['parents'] = $parent_path;
					$data['parents2'] = $parent_path2;
					
					$parentId_array = explode(',', $parent_path);
					$data['parentId'] = $parentId_array[count($parentId_array)-2];
				}
                if($Config->save($data)){
	...
	public function bank(){
    	...
    	$username = I('username');
        $str = 'b.uid=m.uid and l.id=b.bankId and l.isDelete=0 and b.enable=1 and b.admin=0';
    	if($username){
            $str = $str." and m.username like '%".(string)$username."%'";
		}
        if(I('uid')){
            $str = $str." and m.uid=".I('uid');
        }
        $list = $Model->table('__MEMBER_BANK__ b,__BANK_LIST__ l,__MEMBERS__ m')->where($str)->select();
