...
class ScoreController extends AdminController {
	...
	public final function addgoods(){
		if(IS_POST){
			$data = [
				'title' => I('title'),
				'content' => I('content'),
				...
				'stopTime' => strtotime(I('stopTime')),
				'enable' => I('enable'),
				'intoTime' => time(),
			];
			if($data){
				if(M('score_goods')->add($data)){
					$this->success('新增商品成功', U('score/goodslist'));
				} else {
					$this->error('新增商品失败');
				}
			} else {
				$this->error(M('score_goods')->getError());
			}
		}else{
			$this->display();
		}
	...
	public final function updateGoods(){
		if(IS_POST)
		{
            ...
            if($data){
                if(M('score_goods')->where(array('id'=>I('id')))->save($data)){
			...
			$goods = M('score_goods')->where(array('id'=>I('id')))->find();
