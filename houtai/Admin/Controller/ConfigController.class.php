...
class ConfigController extends AdminController {
	...
	public final function bank(){
		...
		$list = $Model->table('__MEMBER_BANK__ b,__BANK_LIST__ l')->where('l.id=b.bankId and b.admin=1')->field('b.*,l.name as bankName')->select();
	...
	public function level(){
		if(IS_POST){
			$map['id'] = I('id');
			$map['levelName'] = I('levelName');
			$map['minScore'] = I('minScore');
			$map['maxToCashCount'] = I('maxToCashCount');
			if(M('member_level')->save($map)){
				$str2 = implode(',',$map);
				$this->addLog(14 , $map['id'], $str2);
				$this->success('修改等级成功', U('level'));
			}else {
				$this->error('修改等级失败');
			}
		}else {
			
			$list = M('member_level')->where(array())->order('id')->select();			
			$this->recordList($list);
			$this->meta_title = '用户等级';
			$this->display();
		}
	}
	...
	public function top(){
		if(IS_POST){
			...
			if(M('top')->save($map)){
				//$str2 = implode(',',$map);
				//$this->addLog(14 , $map['id'], $str2);
				$this->success('修改成功', U('top'));
			}else {
				$this->error('修改失败');
			}
		}else {
			
			$list = M('top')->where(array())->order('id')->select();			
			$this->recordList($list);
			$this->meta_title = '虚拟排行';
			$this->display();
		}
	}
