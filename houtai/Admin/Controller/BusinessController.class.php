...
class BusinessController extends AdminController {
	...
	public final function cash(){
		...
		//dump($para);
		// 用户限制
		if(strlen($para['username'])>0){
			$userwhere=" and m.username like '%".$para['username']."%'";
		}
		// 用户ID限制
		if($para['uid']){
			$userwhere=" and m.uid={$para['uid']}";
		}
		...
		$list = $Model->table('__MEMBERS__ m,__MEMBER_CASH__ c,__BANK_LIST__ l')->where('m.uid=c.uid and l.id=c.bankId and c.isDelete=0'.$userwhere.$timeWhere)->order('c.id desc')->field('c.*,l.name as name,l.home as home,m.username as uName, m.parents2 as parents2')->order('c.id desc')->select();
	...
	public final function recharge(){
		...
		$Model = new \Think\Model();
		$list = $Model->table('__MEMBER_RECHARGE__ r, __MEMBERS__ s')->where('r.isDelete=0 and s.uid=r.uid '.$userwhere.$rechargeIdWhere.$timeWhere.$typeWhere)->order('r.id desc')->field('r.*, s.parents2 as parents2')->select();
		//dump($Model->getLastSql());
	...
	//代理充值
	public final function to_recharge2(){
		if(IS_POST){
			$this->user = session('user_auth');
			$uid=0;
			if(I('user')==1){
			...
			// 查找用户信息
			if(I('user')==1){
				$user=M('members')->field('uid,coin')->where(array('uid'=>$uid))->find();
			}else{
				$user=M('members')->field('uid,coin')->where(array('username'=>$uid))->find();
			}
			if(!$user) $this->error('用户不存在');
			if($user['type']==0) $this->error('此用户不是代理');			
			
			$user['coin']=$user['coin']+$amount;
			if($dataId = M('members')->save($user))
			{
				$this->success('新增充值成功', U('business/recharge'));				
			}
	...
	public final function hemai(){
		...
		if(I('fromTime') && I('toTime')){
			$fromTime=strtotime(I('fromTime'));
			$toTime=strtotime(I('toTime'))+24*3600;
			$where['actionTime'] = array('between',array($fromTime,$toTime));
			$map_hm['actionTime'] = array('between',array($fromTime,$toTime));
		}elseif(I('fromTime')){
			$fromTime=strtotime(I('fromTime'));
			$where['actionTime'] = array('egt',$fromTime);
			$map_hm['actionTime'] = array('egt',$fromTime);
		}elseif(I('toTime')){
			$toTime=strtotime(I('toTime'))+24*3600;
			$where['actionTime'] = array('elt',$toTime);
			$map_hm['actionTime'] = array('elt',$toTime);
		}else{
			$where['actionTime'] = array('gt',strtotime('00:00'));
			$map_hm['actionTime'] = array('gt',strtotime('00:00'));
		}
		// 用户名限制
	...
	public final function hemaiInfo(){
		...
		$hemai=M('bets_hemai')->where(array('id'=>I('hmid')))->find();
		$bets=M('bets')->where(array('hmId'=>$hemai['id']))->order('id')->select();
		...
		$this->display('Business/hemai_detail');
	}
	//投注详单
	...
	public final function updateBet(){
		if(IS_POST){
			...
			if($data['lotteryNo']) $this->error('这单子已经开奖了。');
