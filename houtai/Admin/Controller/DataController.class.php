...
class DataController extends AdminController {
		...
		else
		{
			...
			foreach($list as $var)
			{
				if($type==1){
					...
					// PK10
					$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2007-11-11'))/3600/24+$var['actionNo']-3793;
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
					
				}else if($type==24){
					// 快8
					$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2004-09-19'))/3600/24+$var['actionNo']-2583;;
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
					...
					//韩国1.5
					$number = date('Ymd', $date).($var['actionNo']+39);
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
				}else if($type==61){
					//加拿大3.5
					$number = 398*(strtotime(date('Y-m-d', $date))-strtotime('2007-11-11'))/3600/24+$var['actionNo']+757938;
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
				}else if($type==62){
					//台湾5
					$number = 203*(strtotime(date('Y-m-d', $date))-strtotime('2007-11-11'))/3600/24+$var['actionNo']+105322183;
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
				}else if($type==63){
					//北京5
					$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2007-11-11'))/3600/24+$var['actionNo']+201636;
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
				} else if($type==50){
					//幸运28
					$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2004-09-19'))/3600/24+$var['actionNo']-3856;
					$data = M('data')->where(array('type'=>$type,'number'=>$number))->find();
				}
				else{
	...
	private $node_access='http://localhost:8804';	// node访问基本路径
